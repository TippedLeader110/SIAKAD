<!-- ini navbar-kiri-siswa-->

<div class="col-md-3 .navbar-fixed-left" style="background-color: #8DB3DA;margin-top: -22px;border-style: solid;border-color: black;">
      <div class="profile" align="center">
        <div class="circle">
          <img src="/icon-profile.png" style="max-width: 100px; max-height: 100px;">
        </div>
        <h4 align="center" style="margin-top: 20px; color: white;"><b>Selamat Datang,</b></h4>
        <div class="field-wel">
          <h4 align="center">
            <b>nama_pengguna</b>
          </h4>
        </div>
        <div class="field-menu">
          <div class="row ">
              <div style="width: 200px; height: 33px; background: #ebebe0; margin-left: 50px; margin-top: 10px; margin-bottom: 5px; padding: 5px 5px 5px 20px;">
                <h5 style="color: black; text-align: left;" class="aktif">Beranda</h5>
               </div>
                <div style="width: 200px; height: 33px; background: #ebebe0; margin-left: 50px; margin-top: 10px; margin-bottom: 5px; padding: 5px 5px 5px 20px;">
                <h5 style="color: black; text-align: left;">Daftar Nilai</h5>
               </div>
               <div style="width: 200px; height: 33px; background: #ebebe0; margin-left: 50px; margin-top: 10px; margin-bottom: 5px; padding: 5px 5px 5px 20px;">
                <h5 style="color: black; text-align: left;">Rapot</h5>
               </div>
               <div style="width: 200px; height: 33px; background: #ebebe0; margin-left: 50px; margin-top: 10px; margin-bottom: 5px; padding: 5px 5px 5px 20px;">
                <h5 style="color: black; text-align: left;">Rekap Absensi</h5>
               </div>
               <div style="width: 200px; height: 33px; background: #ebebe0; margin-left: 50px; margin-top: 10px; margin-bottom: 5px; padding: 5px 5px 5px 20px;">
                <h5 style="color: black; text-align: left;">Daftar Guru</h5>
               </div>
               <div style="width: 200px; height: 33px; background: #ebebe0; margin-left: 50px; margin-top: 10px; margin-bottom: 5px; padding: 5px 5px 5px 20px;">
                <h5 style="color: black; text-align: left;">Profil</h5>
               </div>


          </div>
        </div>
      </div>
      
    </div>